...
public class RtcSystemUnity : IRtcSystem
{
		...
		RtcOutputSink initialAudioOutput, RtcInputSource initialVideoInput, string fmLicenseKey)
	{
		if (!fmInitialized)
		{
			...
			FM.LiveSwitch.Log.LogLevel = SystemSettings.EnableVerboseLogging ? LogLevel.Debug : LogLevel.Warn;
			FM.LiveSwitch.Log.RegisterProvider(new DebugLogProvider(FM.LiveSwitch.Log.LogLevel));
		}
		...
		if(buildingForMagicLeap)
		{
			...
			//this.systemRecvMode = this.systemRecvMode & ~RtcModeEnum.Video;
		...
		// handle local texture passthrough
	}
